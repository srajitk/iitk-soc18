<?php
	...
	for ($food = 1; $food <=180; $food++) {
		...
		$avgQuery = "SELECT  AVG(`orders_placed`.`cost_eff`) AS `avg`  FROM `orders_placed` WHERE `food` = $food AND `date_deliver` >= '$start' AND `date_deliver` <= '$end'";
		$avgResult = mysqli_query($cxn, $avgQuery) or die (mysqli_error($cxn));
		while ($avgRow = mysqli_fetch_assoc($avgResult)){
			$avgCF = $avgRow['avg'];
			if (!is_null($avgCF)){
				$query = "SELECT `orders_placed`.`orderid`, `orders_placed`.`pre_img_val`/100.0, `orders_placed`.`del_t`*`item_details_tbl`.`perishablity`  / 86400.0  `time_loss`, `orders_placed`.`cost_eff` / ".$avgCF.", `farmer_tbl`.`value`  FROM `orders_placed`, `farmer_tbl`, `item_details_tbl` WHERE `food` = '$food' AND `date_deliver` >= '$start' AND `date_deliver` <= '$end' AND `farmer_tbl`.`user_id` = `farmer_id` AND `item_details_tbl`.`item_no` = `orders_placed`.`food`  ORDER BY `cost_eff` DESC ";
				...
				$rankQuery = "SET @rno = 0; UPDATE `orders_placed` SET `rank_in_q` = (@rno:=@rno+1) WHERE `food` = ".$food." AND `date_deliver` >= '$start' AND `date_deliver` <= '$end'  ORDER BY `qval` DESC";	
